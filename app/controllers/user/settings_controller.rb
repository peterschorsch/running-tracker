class User::SettingsController < User::UsersController
  before_action :set_user, only: [:edit, :update, :update_password]
  before_action only: [:update, :update_password] do
    website_viewer_authorization(user_settings_edit_path(current_user))
  end
  ...
  def update
    respond_to do |format|
      if current_user.update(user_params)
        format.html { redirect_to user_settings_edit_path(current_user), notice: update_notice("Your Profile") }
  ...
  def update_password
    respond_to do |format|
      if current_user.update_with_password(user_password_params)
        format.html { redirect_to user_settings_edit_path(current_user), notice: update_notice("Your Password") }
      else
        format.html { render :edit }
        format.json { render json: current_user.errors, status: :unprocessable_entity }
      end
  ...
  private
    def set_user
      ...
      if current_user != @user || current_user.is_archived?
        ...
        redirect_to user_settings_edit_path(current_user)
      end
    end
