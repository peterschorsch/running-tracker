...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:default => true, :shoe_brand_id => adidias_id, :model => model, :color_way => color_way, :forefoot_stack => 17, :heel_stack => 27, :heel_drop => 10, :previous_mileage => 107.9, :weight => "7.9", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 8, 12), :first_used_on => Date.new(2019, 10, 6),
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => brooks_id, :model => model, :color_way => color_way, :forefoot_stack => 16, :heel_stack => 26, :heel_drop => 10, :weight => "9.5", :previous_mileage => BigDecimal('700'), :mileage_total => BigDecimal('700'), :size => "9", :shoe_type => "Stability", :purchased_on => Date.new(2018, 3, 17), :first_used_on => Date.new(2018, 3, 18), retired: true, :retired_on => Date.new(2019, 4, 12), 
  :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => brooks_id, :model => model, :color_way => color_way, :forefoot_stack => 17, :heel_stack => 27, :heel_drop => 10, :weight => "9.4", :previous_mileage => BigDecimal('239.9'), :mileage_total => BigDecimal('239.9'), :size => "9", :shoe_type => "Stability", :purchased_on => Date.new(2019, 2, 6), :first_used_on => Date.new(2019, 4, 15), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => ShoeBrand.named("New Balance").id, :model => model, :color_way => color_way, :forefoot_stack => 18, :heel_stack => 28, :heel_drop => 10, :previous_mileage => BigDecimal('334.8'), :mileage_total => BigDecimal('334.8'), :weight => "10.5", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 10, 9), :first_used_on => Date.new(2019, 10, 10), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => nike_id, :model => model, :color_way => color_way, :forefoot_stack => 28, :heel_stack => 36, :heel_drop => 8, :previous_mileage => BigDecimal('66.3'), :mileage_total => BigDecimal('66.3'), :weight => "8.9", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 10, 31), :first_used_on => Date.new(2019, 12, 12), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => on_id, :model => model, :color_way => color_way, :forefoot_stack => 16, :heel_stack => 22, :heel_drop => 6, :previous_mileage => BigDecimal('141.1'), :mileage_total => BigDecimal('141.1'), :weight => "8.1", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 9, 10), :first_used_on => Date.new(2019, 12, 12), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => on_id, :model => model, :color_way => color_way, :forefoot_stack => 22, :heel_stack => 29, :heel_drop => 7, :previous_mileage => BigDecimal('16.4'), :mileage_total => BigDecimal('16.4'), :weight => "10.1", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2020, 1, 29), :first_used_on => Date.new(2020, 1, 31), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => on_id, :model => model, :color_way => color_way, :forefoot_stack => 22, :heel_stack => 29, :heel_drop => 7, :previous_mileage => BigDecimal('6.1'), :mileage_total => BigDecimal('6.1'), :weight => "10.2", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2020, 11, 14), :first_used_on => Date.new(2020, 6, 28), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
puts @shoe.inspect
model = "Cloudswift"
@shoe = Shoe.create_with(:shoe_brand_id => on_id, :model => model, :color_way => color_way, :forefoot_stack => 22, :heel_stack => 29, :heel_drop => 7, :previous_mileage => BigDecimal('681.4'), :mileage_total => BigDecimal('681.4'), :weight => "10.2", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 4, 1), :first_used_on => Date.new(2019, 8, 18), :retired => true, :retired_on => Date.new(2020, 7, 12), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => on_id, :model => model, :color_way => color_way, :forefoot_stack => 22, :heel_stack => 29, :heel_drop => 7, :previous_mileage => BigDecimal('38.1'), :mileage_total => BigDecimal('38.1'), :weight => "10.2", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 9, 2), :first_used_on => Date.new(2019, 9, 13), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @website_viewer.id)
