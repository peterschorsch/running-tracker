...
if @obligations.of_date(start_time).empty?
	@obligation = Obligation.find_or_create_by(name: "Attend Meeting", start_time: start_time, end_time: DateTime.current, city: "Los Angeles", state_id: State.find_by_abbr("CA").id, user_id: @my_admin_user.id, obligation_color_id: @obligation_color.id)
	puts @obligation.inspect
end
puts ""
puts ""