User.exclude_viewer_accounts.each do |user|
  ...
  monthly_total_id = MonthlyTotal.of_month(run_date).id
  @run = Run.find_or_create_by(name: "Bucktown 5K Run", start_time: run_date.in_time_zone("Central Time (US & Canada)"), hours: "0", minutes: "18", seconds: "4", pace: "5:49", city: "Chicago", notes: "Bib#: 4291", gear_id: Gear.find_shoe("Adios 4").id, planned_mileage: BigDecimal('3.1'), mileage_total: BigDecimal('3.1'), elevation_gain: BigDecimal('0'), state_id: illinois_state_id, run_type_id: run_type_id, user_id: user.id, monthly_total_id: monthly_total_id, personal_best: true, completed_run: true, active_run: true)
  puts @run.inspect

  run_date = DateTime.new(2019, 10, 13, 7, 30, 0)
  monthly_total_id = MonthlyTotal.of_month(run_date).id
  @run = Run.find_or_create_by(name: "Bank of America Chicago Marathon", start_time: run_date.in_time_zone("Central Time (US & Canada)"), hours: "2", minutes: "43", seconds: "08", pace: "6:14", notes: "Bib#: 11184", city: "Chicago", gear_id: Gear.find_shoe("Adios 4").id, planned_mileage: BigDecimal('26.2'), mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('100'), state_id: illinois_state_id, run_type_id: run_type_id, user_id: user.id, monthly_total_id: monthly_total_id, personal_best: true, completed_run: true, active_run: true)
  puts @run.inspect

  run_date = DateTime.new(2019, 11, 3, 9, 42, 0)
  monthly_total_id = MonthlyTotal.of_month(run_date).id
  @run = Run.find_or_create_by(name: "TCS New York City Marathon", start_time: run_date.in_time_zone("Eastern Time (US & Canada)"), hours: "3", minutes: "7", seconds: "16", pace: "7:09", notes: "Bib#: 2052", city: "New York City", gear_id: Gear.find_shoe_with_color("Cloudswift", "Rock/Slate").id, planned_mileage: BigDecimal('26.2'), mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('830'), state_id: State.find_by_abbr("NY").id, run_type_id: run_type_id, user_id: user.id, monthly_total_id: monthly_total_id, completed_run: true, active_run: true)
  puts @run.inspect

  puts "----------2020----------"
  run_date = DateTime.new(2020, 3, 8, 6, 55, 0)
  monthly_total_id = MonthlyTotal.of_month(run_date).id
  @run = Run.find_or_create_by(name: "Los Angeles Marathon", start_time: run_date.in_time_zone("Pacific Time (US & Canada)"), hours: "2", minutes: "48", seconds: "35", pace: "6:26", notes: "Bib#: 1356", city: "Los Angeles", gear_id: Gear.find_shoe("Adios 4").id, planned_mileage: BigDecimal('26.2'), mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('850'), state_id: State.find_by_abbr("CA").id, run_type_id: run_type_id, user_id: user.id, monthly_total_id: monthly_total_id, completed_run: true, active_run: true)
  puts @run.inspect

  two_days_ago = 2.days.ago
  run_date = DateTime.new(two_days_ago.year, two_days_ago.month, two_days_ago.day, 10, 0, 0)
  monthly_total_id = MonthlyTotal.of_month(run_date).id
  @run = Run.find_or_create_by(name: "Testing", start_time: run_date.in_time_zone("Pacific Time (US & Canada)"), hours: "2", minutes: "28", seconds: "23", pace: "7:02", notes: nil, city: "Los Angeles", gear_id: Gear.find_shoe("Adios 4").id, planned_mileage: BigDecimal('20'), mileage_total: BigDecimal('21'), elevation_gain: BigDecimal('1000'), state_id: State.find_by_abbr("CA").id, run_type_id: RunType.named("Long Run").id, user_id: user.id, monthly_total_id: monthly_total_id, completed_run: true, active_run: true)
  puts @run.inspect

  one_day_ago = 1.day.ago
  run_date = DateTime.new(one_day_ago.year, one_day_ago.month, one_day_ago.day, 18, 15, 0)
  monthly_total_id = MonthlyTotal.of_month(run_date).id
  @run = Run.find_or_create_by(name: "Testing 2", start_time: run_date.in_time_zone("Pacific Time (US & Canada)"), hours: 0, minutes: "30", seconds: "00", pace: "7:30", notes: nil, city: "Los Angeles", gear_id: Gear.find_shoe("Adios 4").id, planned_mileage: BigDecimal('5'), mileage_total: BigDecimal('5'), elevation_gain: BigDecimal('252'), state_id: State.find_by_abbr("CA").id, run_type_id: RunType.named("Easy Run").id, user_id: user.id, monthly_total_id: monthly_total_id, completed_run: true, active_run: true)
  puts @run.inspect
  puts ""
