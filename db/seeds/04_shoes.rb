...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => adidias_id, :model => model, :color_way => color_way, :forefoot_stack => 17, :heel_stack => 27, :heel_drop => 10, :previous_mileage => BigDecimal('87.1'), :mileage_total => BigDecimal('87.1'), :weight => "7.9", :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 8, 12), :first_used_on => Date.new(2019, 10, 6),
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @my_admin_user.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => saucony_id, :model => model, :color_way => color_way, :forefoot_stack => 19, :heel_stack => 23, :heel_drop => 8, :weight => "9.4", :previous_mileage => BigDecimal('55.6'), :mileage_total => BigDecimal('55.6'), :size => "8.5", :shoe_type => "Neutral", :purchased_on => Date.new(2019, 6, 23), :first_used_on => Date.new(2019, 7, 21), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @my_admin_user.id)
...
image_file = File.new(image_path)
@shoe = Shoe.create_with(:shoe_brand_id => ShoeBrand.named("Atreyu").id, :model => model, :color_way => color_way, :forefoot_stack => 15.5, :heel_stack => 21.5, :heel_drop => 6, :weight => "6", :previous_mileage => 25.8, :size => "8.5", :shoe_type => "Neutral", :purchased_on =>  Date.new(2020, 12, 1), :first_used_on => Date.new(2020, 12, 23), :retired_on => Date.new(2021, 2, 7),
  :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @my_admin_user.id)
...
model = "Voyager (P2)"
@shoe = Shoe.create_with(:shoe_brand_id => ShoeBrand.named("Atreyu").id, :model => model, :color_way => color_way, :forefoot_stack => 15.5, :heel_stack => 21.5, :heel_drop => 6, :weight => "6", :previous_mileage => 25.8, :size => "8", :shoe_type => "Neutral", :purchased_on =>  Date.new(2021, 1, 28), :first_used_on => Date.new(2021, 2, 9),
  :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way, :user_id => @my_admin_user.id)
