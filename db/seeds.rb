...
image_file = File.new(image_path)
@gear = Gear.create_with(:shoe_brand_id => brooks_id, :model => model, :color_way => color_way, :heel_drop => 10, :weight => 10.5, :mileage => 800, :size => 9, :shoe_type => "Stability", :purchased_on => Date.new(2017, 5, 13), :first_used_on => Date.new(2017, 5, 14), retired: true, :retired_on => Date.new(2018, 3, 17),
  :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way)
...
image_file = File.new(image_path)
@gear = Gear.create_with(:shoe_brand_id => brooks_id, :model => model, :color_way => color_way, :heel_drop => 10, :weight => 9.5, :mileage => 700, :size => 9, :shoe_type => "Stability", :purchased_on => Date.new(2018, 3, 17), :first_used_on => Date.new(2018, 3, 18), retired: true, :retired_on => Date.new(2019, 4, 12), 
  :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way)
...
image_file = File.new(image_path)
@gear = Gear.create_with(:shoe_brand_id => brooks_id, :model => model, :color_way => color_way, :heel_drop => 10, :weight => 9.4, :mileage => 239.9, :size => 9, :shoe_type => "Stability", :purchased_on => Date.new(2019, 2, 6), :first_used_on => Date.new(2019, 4, 15), 
	:image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )).find_or_create_by(:model => model, :color_way => color_way)
...
puts "----------RUNS----------"
puts "----------2017----------"
@run = Run.find_or_create_by(name: "Chicago Chinatown 5K & Youth Run", mileage_total: BigDecimal('3.1'), start_time: DateTime.new(2017, 7, 8, 8, 0, 0).in_time_zone("Central Time (US & Canada)"), hours: "0", minutes: "19", seconds: "59", pace: "6:26", gear_id: Gear.find_shoe("Ravenna 8").id, elevation_gain: BigDecimal('0'), city: "Chicago", state_id: State.find_by_abbr("IL").id, notes: "Bib #: 438", run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Rock 'n' Roll Chicago Half Marathon", mileage_total: BigDecimal('13.1'), start_time: DateTime.new(2017, 7, 15, 6, 30, 0).in_time_zone("Central Time (US & Canada)"), hours: "1", minutes: "33", seconds: "09", pace: "7:27", notes: "Bib#: 1386", city: "Chicago", gear_id: Gear.find_shoe("Ravenna 8").id, elevation_gain: BigDecimal('150'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Fox Valley Marathon", mileage_total: BigDecimal('26.2'), start_time: DateTime.new(2017, 9, 17, 7, 0, 0).in_time_zone("Central Time (US & Canada)"), hours: "3", minutes: "11", seconds: "17", pace: "7:18", notes: "Bib#: 727", city: "St. Charles", gear_id: Gear.find_shoe("Ravenna 8").id, elevation_gain: BigDecimal('326'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Naperville Half Marathon and 5K", mileage_total: BigDecimal('13.1'), start_time: DateTime.new(2017, 10, 22, 7, 0, 0).in_time_zone("Central Time (US & Canada)"), hours: "1", minutes: "25", seconds: "48", pace: "6:33", notes: "Bib#: 3800", city: "Naperville", gear_id: Gear.find_shoe("Ravenna 8").id, elevation_gain: BigDecimal('0'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
puts "----------2018----------"
@run = Run.find_or_create_by(name: "Chicago Spring Half Marathon & 10k", start_time: DateTime.new(2018, 5, 20, 7, 50, 0).in_time_zone("Central Time (US & Canada)"), hours: "0", minutes: "38", seconds: "21", pace: "6:11", notes: "Bib#: 8111", city: "Chicago", gear_id: Gear.find_shoe("Ravenna 9").id, mileage_total: BigDecimal('6.2'), elevation_gain: BigDecimal('0'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id, personal_best: true)
puts @run.inspect
@run = Run.find_or_create_by(name: "Chicago Chinatown 5K & Youth Run", start_time: DateTime.new(2018, 7, 14, 8, 0, 0).in_time_zone("Central Time (US & Canada)"), hours: "0", minutes: "18", seconds: "04", pace: "5:49", notes: "Bib#: 36", city: "Chicago", gear_id: Gear.find_shoe("Ravenna 9").id, mileage_total: BigDecimal('3.1'), elevation_gain: BigDecimal('0'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Rock 'n' Roll Chicago Half Marathon", start_time: DateTime.new(2018, 7, 21, 6, 30, 0).in_time_zone("Central Time (US & Canada)"), hours: "1", minutes: "21", seconds: "18", pace: "6:12", notes: "Bib#: 1346", city: "Chicago", gear_id: Gear.find_shoe("Ravenna 9").id, mileage_total: BigDecimal('13.1'), elevation_gain: BigDecimal('0'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Fox Valley Marathon", start_time: DateTime.new(2018, 9, 16, 7, 0, 0).in_time_zone("Central Time (US & Canada)"), hours: "2", minutes: "51", seconds: "48", pace: "6:33", notes: "Bib#:  622", city: "St. Charles", gear_id: Gear.find_shoe("Ravenna 9").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('326'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Bank of America Chicago Marathon", start_time: DateTime.new(2018, 10, 7, 7, 30, 0).in_time_zone("Central Time (US & Canada)"), hours: "2", minutes: "46", seconds: "11", pace: "6:21", notes: "Bib#: 6084", city: "Chicago", gear_id: Gear.find_shoe("Ravenna 9").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('242'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
...
puts @run.inspect
puts "----------2019----------"
@run = Run.find_or_create_by(name: "Boston Marathon", start_time: DateTime.new(2019, 4, 15, 10, 02, 0).in_time_zone("Eastern Time (US & Canada)"), hours: "2", minutes: "57", seconds: "08", pace: "6:46", notes: "Bib#: 1183", city: "Boston", gear_id: Gear.find_shoe("Ravenna 10").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('655'), state_id: State.find_by_abbr("MA").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Rock 'n' Roll Chicago Half Marathon", start_time: DateTime.new(2019, 7, 21, 6, 30, 0).in_time_zone("Central Time (US & Canada)"), hours: "1", minutes: "19", seconds: "44", pace: "6:05", notes: "Bib#: 1175", city: "Chicago", gear_id: Gear.find_shoe("Kinvara 10").id, mileage_total: BigDecimal('13.1'), elevation_gain: BigDecimal('0'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id, personal_best: true)
puts @run.inspect
@run = Run.find_or_create_by(name: "Fox Valley Marathon", start_time: DateTime.new(2010, 9, 22, 7, 0, 0).in_time_zone("Central Time (US & Canada)"), hours: "2", minutes: "49", seconds: "34", pace: "6:28", notes: "Bib#: 347", city: "St. Charles", gear_id: Gear.find_shoe("Kinvara 10").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('326'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Bucktown 5K Run", start_time: DateTime.new(2019, 10, 6, 8, 30, 0).in_time_zone("Central Time (US & Canada)"), hours: "0", minutes: "18", seconds: "4", pace: "5:49", city: "Chicago", notes: "Bib#: 4291", gear_id: Gear.find_shoe("Adios 4").id, mileage_total: BigDecimal('3.1'), elevation_gain: BigDecimal('0'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id, personal_best: true)
puts @run.inspect
@run = Run.find_or_create_by(name: "Bank of America Chicago Marathon", start_time: DateTime.new(2019, 10, 13, 7, 30, 0).in_time_zone("Central Time (US & Canada)"), hours: "2", minutes: "43", seconds: "08", pace: "6:14", notes: "Bib#: 11184", city: "Chicago", gear_id: Gear.find_shoe("Adios 4").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('100'), state_id: State.find_by_abbr("IL").id, run_type_id: RunType.named("Race").id, user_id: user_id, personal_best: true)
puts @run.inspect
@run = Run.find_or_create_by(name: "TCS New York City Marathon", start_time: DateTime.new(2019, 11, 3, 9, 42, 0).in_time_zone("Eastern Time (US & Canada)"), hours: "3", minutes: "7", seconds: "16", pace: "7:09", notes: "Bib#: 2052", city: "New York", gear_id: Gear.find_shoe_with_color("Cloudswift", "Rock/Slate").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('830'), state_id: State.find_by_abbr("NY").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
puts "----------2020----------"
@run = Run.find_or_create_by(name: "Los Angeles Marathon", start_time: DateTime.new(2020, 3, 8, 6, 55, 0).in_time_zone("Pacific Time (US & Canada)"), hours: "2", minutes: "48", seconds: "35", pace: "6:26", notes: "Bib#: 1356", city: "Los Angeles", gear_id: Gear.find_shoe("Adios 4").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('850'), state_id: State.find_by_abbr("CA").id, run_type_id: RunType.named("Race").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Testing", start_time: DateTime.new(2020, 9, 5, 10, 0, 0).in_time_zone("Pacific Time (US & Canada)"), hours: "2", minutes: "48", seconds: "35", pace: "6:26", notes: "Bib#: 1356", city: "Los Angeles", gear_id: Gear.find_shoe("Adios 4").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('850'), state_id: State.find_by_abbr("CA").id, run_type_id: RunType.named("Long Run").id, user_id: user_id)
puts @run.inspect
@run = Run.find_or_create_by(name: "Testing 2", start_time: DateTime.new(2020, 9, 8, 18, 15, 0).in_time_zone("Pacific Time (US & Canada)"), hours: "2", minutes: "48", seconds: "35", pace: "6:26", notes: "Bib#: 1356", city: "Los Angeles", gear_id: Gear.find_shoe("Adios 4").id, mileage_total: BigDecimal('26.2'), elevation_gain: BigDecimal('850'), state_id: State.find_by_abbr("CA").id, run_type_id: RunType.named("Easy Run").id, user_id: user_id)
puts @run.inspect
puts ""
puts ""
